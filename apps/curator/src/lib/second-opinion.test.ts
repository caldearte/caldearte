...
import { applySecondOpinion, secondOpinionVetoes } from "./second-opinion.js";
...
test("secondOpinionVetoes flags an approval the second model rejected on scope", () => {
  ...
  assert.deepEqual(secondOpinionVetoes(real, shadow), [
    { sourceUrl: "https://www.instagram.com/p/AAA/", reasoning: "Muestra documental/patrimonial, no arte visual.", rejectionAxis: null },
  ]);
});

test("secondOpinionVetoes carries the second model's rejection axis", () => {
  ...
  assert.equal(secondOpinionVetoes(real, shadow)[0].rejectionAxis, "religion");
});

test("secondOpinionVetoes never vetoes on a code-filter rejection, a missing item, or a split verdict", () => {
  const real = [
...
test("secondOpinionVetoes ignores Haiku's own rejections", () => {
...
test("applySecondOpinion sends only the approved items, rewrites the vetoed candidates as rejections, and keeps the rest", async () => {
  const candidates = [
  ...
  const vetoed = await applySecondOpinion({ client: { messages: { create: async () => ({}) } } as never, model: "test-model" }, candidates, items, curateFn);
  ...
  assert.match(candidates[1].curationReasoning, /^\[VETO segunda opinión test-model\] Muestra documental, no arte visual\. — Haiku había aprobado: /);
  assert.equal(candidates[2].curationReasoning, "Concierto.");
});

test("applySecondOpinion keeps every approval when the second model's call fails", async () => {
  ...
  const vetoed = await applySecondOpinion({ client: {} as never, model: "test-model" }, candidates, [item("https://www.instagram.com/p/AAA/")], curateFn);
