// Deliberate small duplicate of apps/curator/src/lib/apify-cost-split.ts's
// own logic — curator and web are separate deployable packages (curator is
// a plain Node script, not a Next.js app importing from apps/web), so this
...
// The daily amounts stored by apps/curator's apify-usage-snapshot are
// GROSS usage cost, not already net of the free credit — this file is
// what splits each day into its free-covered portion vs. its real
// (billed) portion, so the admin cost views can show "this is real
// money, this isn't" instead of quietly treating $5/cycle of free usage
// as if it cost something.
...
// there's nothing to keep in sync day to day. apps/curator's
// apify-usage-snapshot checks Apify's own response against it on every
// run and logs loudly if they ever disagree, which is the cheap way to
// find out it moved.
